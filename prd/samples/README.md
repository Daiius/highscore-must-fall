...
**多列レイアウトの読み順は「いちばん左の列を上から下まで全部 → 次の列の最上段」。**
**列数は固定ではない**（生存日数が伸びると 3 列以上になる）。週の見出しが列の末尾に来て中身が
次の列の先頭から始まる（`contracts-04.png` の WEEK 3）ほか、**見出しの下に数行だけ置いて列が尽き、
残りが次の列へ続く**こともある（38 日の run で WEEK 5 が 1 行 + 11 行に割れた）。
読み順を間違えると週がずれる。実運用で W3 の列またぎ誤配を検知した実績があり、38 日の run では
WEEK 5 の 11 行が丸ごと WEEK 4 に混入した。→ worker 側の対処は `prd/04-ingestion.md` §9.3。
...
tree-02 より 12px 狭いだけ（高さは同じ 992）で、`y`/`h` をそのまま流用できた。幅は右端まで取る
