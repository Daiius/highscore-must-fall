...
**UPGRADE HISTORY は列数が可変**（生存日数が伸びると 3 列以上になる）。この画面は列ごとに上から下へ
読み、**列を跨いで並びが連続する**（列の先頭に WEEK 見出しが無ければ前の列の週の続き）。1 枚のまま
読ませるとこの継続を取り違え、**後ろの週が前の週へ丸ごと吸い込まれる**。

> 実測（3 列・38 日の run / 各条件 3〜4 回）: 旧プロンプトは「2列レイアウト」と列数を決め打っており、
> WEEK 5 の 11 行が丸ごと WEEK 4 に混入した。列数を仮定しない文面に直しても 4 回に 1 回再発する。
> **注意書きは確率的にしか効かない。**

そこで worker は、**元画像に加えて「列ごとに切り出して拡大した画像」を併送する**（`image/columns.ts`）。
読む順序が画像の並びで決まるため、曖昧さが構造的に消える。実測で週の誤配と行の欠落がゼロになった。

- **切ったあと拡大する。** 細長い列画像のままだとモデル側のリサイズで字が潰れる
  （実測: `EXTENDED BARREL` を 4/4 で `EXTENDED GARBAGE` と誤読 → 下部の空白を落として 2 倍にすると消えた）。
- **分割は補助であって前提ではない。** 罫線で囲まれた多列レイアウトを検出できたときだけ切り、
  それ以外（結果画面・1 列レイアウト・PNG 以外・読み書き失敗）は**元画像だけで従来どおり**処理する。
  誤って切るより切らない方が安全なので、判定は厳しめに置く。
- **REWARD LEDGER は切らない。** `NAME......12×..300` と数値が右揃えなので、行の途中に縦の空白が
  揃って立ち、列と誤認すると**行そのものが名前・回数・点数に分断される**（実測: 2 列が 3 列に割れた）。
  本文行のうち右端付近まで文字が届く行の割合で見分ける（実測: UPGRADE HISTORY 0.12〜0.19 /
  REWARD LEDGER 0.93〜0.98）。そもそも列を跨ぐ継続の問題は UPGRADE HISTORY 固有で、
  REWARD LEDGER を切る利点が無い。
- **見るのはパネルの内側だけ**、しかも**ボタン区画を除いた本文だけ**。パネルの外には星や飛行中の
  敵機が居て列間の空白を埋める。パネル下部の `CLOSE` ボタンは、2 列レイアウトでは**必ず中央の
  列間ギャップと重なる**（実測: どちらも列を全て見失った）。
- **入力が健全でなければ切らない**（壊れた PNG・扱えない透過指定・大きすぎる画像）。列画像は
  読み取りの**原本**になる（プロンプトが「列画像から読む」と指示する）ので、疑わしい入力から
  作った列画像は誤読に直結する。メモリ上限は入力側だけでなく**拡大後の出力側にも**要る
  ——各列は縦横とも 2 倍になるため、入力が上限内でも出力は最大 4 倍に膨らむ。作りかけて
  OOM になると、この節の「元画像で続ける」フォールバックにも到達できない。
- **元画像も必ず渡す。** `images[]` の section 分類は元画像の index に対して行わせる
  （§9.1 のとおり worker がその index で `run_image` を引き直すため）。誤検出で列画像が付いても、
  元画像がある限り読み取り自体は壊れない。
- 依存を増やさないため PNG の最小コーデックを worker 内に持つ（`image/png.ts`）。
  UPGRADE HISTORY は**色に意味がある**（リロール = 灰色斜体）ので RGB のまま扱う。
