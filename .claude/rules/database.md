...
- **`evidence` はフル解像度スクショでなく証拠シートでよい**（名前が読める帯だけを切り出して連結した1枚。`scripts/evidence-sheet.mjs` が生成。実測 10.1MB→132KB）。加工物を原典にする条件は「変換が決定論的（`--verify` で画素一致を再現）」＋「原本へ遡れる（manifest に sha256）」の2つ。原本はリポジトリに入れない（`.claude-personal/samples-raw/`）。シートを指す `evidence` は、その名前が manifest に宣言されているかまでテストが見る。**色に意味がある UPGRADE HISTORY には使えない**（リロールの灰色斜体が落ちる）。→ 正典は [`prd/08-catalog-lifecycle.md`](../../prd/08-catalog-lifecycle.md) §3.1。
- **カタログ verify はまず SIGN CONTRACTS 画面のツリーを当たる**。未取得ノードもホバーで名称・説明・前提が読めるので、「その名前が出る run を回す」必要が無い（prd/samples/README §4）。
...
- **種別 `kind`**（`upgrade_catalog` のみ）: `contract`（既定） / `opportunity_upgrade`(OU)。OU はゲーム内で `LIMITED OPPORTUNITY` として提示される。**記録は通常アップグレードと同じ**（`entry_type` に第3種は足さない）。`kind` は `unverified` 同様、後から人手で付与/検証する「一応後から区別できる」程度の属性。
  - 解放は run 跨ぎのスコア累計によるメタ進行だが、**すぐ飽和するので交絡要因として扱わない**。**1 run に出るのは最大 8 個**で、run ごとに抽選される。**取得タイミングとトレードオフは通常 contract と同じ**（OU を取る＝その機会に他を取らない）。→ 事実の正典は [`prd/01-game-domain.md`](../../prd/01-game-domain.md) §3.2。
