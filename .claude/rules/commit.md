...
- **レビュー bot**: PR に `@highscore-must-fall-reviewer review` とコメントすると自動レビューが走る（通常10分以内に応答）。マージ前に必ず1回は実行し、指摘に対応 → 再度 `review` コメント、のループで指摘が無くなってからマージする。
  ...
  - **bot のスクショ誤読指摘**: review bot はスクショ自動解析 worker と**同じモデル系統＝同じ OCR 誤読を再現する**。カタログ名の綴りを「スクショと違う」と指摘されたら、まず `catalog-data.ts` の `evidence` を見る。**`evidence` がある名前への指摘は却下してよい**（返信コメントに `prd/samples/<evidence>.png` のパスと画像内の位置を書く。実在はテストが強制済み）。**`evidence` が無い（仮登録）名前への指摘は正当な可能性がある**が、bot の読みを鵜呑みにして seed を書き換えず、疑義台帳に記録してスクショで決着させる。繰り返されるなら説得を続けず、ユーザー判断で override マージ。→ 正典は [`prd/08-catalog-lifecycle.md`](../../prd/08-catalog-lifecycle.md) §8。
