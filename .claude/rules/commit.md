...
- **レビュー bot**: PR に `@highscore-must-fall-reviewer review` とコメントすると自動レビューが走る（通常10分以内に応答）。マージ前に必ず1回は実行し、指摘に対応 → 再度 `review` コメント、のループで指摘が無くなってからマージする。
  ...
  - **bot のスクショ誤読指摘**: review bot はスクショ自動解析 worker と**同じモデル系統＝同じ OCR 誤読を再現する**。カタログ名の綴りを「スクショと違う」と指摘されたら、**bot 自身が誤読している可能性を先に疑う**。
    - **現行の手順**（`evidence` 実装前）: `catalog-data.ts` の `verified` を見る。**`verified: true` の名前への指摘は、`prd/samples/` の画像を人が目視して確認したうえで却下してよい**（返信コメントに画像のパスと位置を書く）。`verified: false`（仮登録）の名前への指摘は正当な可能性があるが、bot の読みを鵜呑みにして seed を書き換えず、疑義台帳に記録してスクショで決着させる。
    - **`evidence` 実装後**（後続 PR。[`prd/08`](../../prd/08-catalog-lifecycle.md) §3）: どの画像と突合したかが seed に書かれ、テストが実在を強制するので、目視せずに `prd/samples/<evidence>.png` を示して却下できる。
    - どちらの段階でも、指摘が繰り返されるなら説得を続けず、ユーザー判断で override マージ。→ 正典は [`prd/08-catalog-lifecycle.md`](../../prd/08-catalog-lifecycle.md) §8。
