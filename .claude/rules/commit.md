...
- **レビュー bot**: マージ前に必ず1回は実行し、指摘に対応 → 再レビュー、のループで指摘が無くなってからマージする。
  - **起動コマンド（bot の名前）・完了判定・待機手順の正典はユーザースコープの `~/.claude/CLAUDE.md`**。bot は複数リポジトリ共通で、**名前も変わる**ため、ここには書かない（かつて書いていた `@highscore-must-fall-reviewer` は古い名前で、無反応の原因になった）。
  ...
  - **レビュー中に PR の内容が変わると bot は中断する**（無応答に見える）。レビューを投げる前に push を済ませ、**応答が返るまで push しない**。追加の修正は応答を受け取ってから積み、再度投げ直す。
